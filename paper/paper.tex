...
Alice's gameboard, Bob can try to use $\code{(obs}\ \blue{e_\ell}\ \blue{e_\text{key}}\ 
\blue{e_\text{fac}}\code{)}$ form, which takes a label, a key, and a faceted value.
Explicit observation projects a single label $\blue{e_\ell}$ in faceted value $\blue{e_\text{fac}}$ 
to either its positive or negative facet, depending on whether the policy associated 
with $\blue{e_\ell}$ returns true for key $\blue{e_\text{key}}$. If Bob tries to observe 
...
$\blue{+\ell}$ facet). This is because, for the duration of the app ``split'',
