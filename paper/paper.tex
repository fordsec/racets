...
\zw{Facet construction allows the users to construct a facet value that
consists of a policy, a private/positive variable that can be a basic value
or another facet value but is hidden away from other users, and a public/negative 
variable. }

\zw{Label creation dynamically creates a label binding to a given policy.
A policy consists of a policy variable and a policy body/expression. 
When a lable is invoked in the faceted executions, the policy will be called
to evaluate the given faceted value. When the policy is invoked, the policy
body (e.g. lambda function) evaluates to a boolean, which indicates whether
to observe the positive or the negative branch of a faceted value. This allows
the users to protect their data via choosing what data to be seen by others and 
hiding the sensitive data.}

\zw{Facet observation }
...
Racets defines its \code{box} macro, along with \code{unbox} and
