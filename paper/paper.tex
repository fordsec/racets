...
policy as it enters the system, and policies propagate through the
program alongside data as computation progresses. Policy-agnostic
programming is often implemented using \emph{faceted execution}, a
dynamic information-flow monitor encoding sensitive values as
\emph{facets}: decision trees specifying different views of that
data. For example, the facet \facet{Alice}{\code{true}}{\code{false}}
represents a value that should appear to Alice as \blue{\code{true}} and to
everyone but Alice as \blue{\code{false}}.

Faceted execution propagates facets by extending core linguistic
primitives (such as function application) to handle faceted
values. For example, consider the application \blue{\code{(x true)}} where
...
\blue{\code{ppapp}} expects a procedure rather than a facet.  The correct way
to interpret function application on facets is to distribute the
application through each branch, producing
\facet{Alice}{\code{true}}{\code{false}}. Many other core forms (such
as \code{if}, \code{set!}, etc...) require similar changes.
...
with how privacy policies are enforced. And yet, we believe that the
relatively foundational linguistic changes required to enable faceted
execution have hindered implementations thusfar. For example, the
first implementation of faceted execution (by Austin et
al.~\cite{Austin:12}) extended a JavaScript interpreter to account for
faceted values. Other implementations use monads~\cite{Schmitz:2016}
or rely upon third-party macro systems~\cite{Yang:2016}. So far, we
know of no implementation of faceted execution that works seamlessly
alongside the language. By contrast, Racket has a powerful macro
system, allowing essentially any linguistic form (including function
application) to be extended with arbitrary semantics.
...
\begin{displaymath}
  \begin{array}{lcrcl}
    ...
    & & & \fcol{\mid} & \fcol{(\code{let-label} ~ \ell ~ e ~ e)} \\
    & & & \fcol{\mid} & \fcol{(\code{facet} ~ e ~ e ~ e)} \\
    & & & \fcol{\mid} & \fcol{(\code{obs} ~ e ~ e ~ e)} \\
  \end{array}
\end{displaymath}
\label{fig:syntax}
