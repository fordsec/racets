...
\begin{figure*}
      ...
      \begin{minipage}{.5\textwidth}
        \begin{displaymath}
          \begin{array}{rcrcl}
            \blue{\alpha} & \blue{\in} & \blue{\text{addr}} & \blue{=} & \blue{\ldots} \\
            \blue{bv} & \blue{\in} & \blue{\text{base-val}} & \blue{::=} & \blue{c \mid \alpha \mid \langle \lambda x.~ e, \rho \rangle \mid \star}\\
            \blue{fv} & \blue{\in} & \blue{\text{facet-val}} & \blue{::=} & %% \blue{bv \mid \facet{\alpha}{fv}{fv}}
            \\
          \end{array}
        \end{displaymath}
      \end{minipage}
    \end{tabular}
    \\
    \hfill \boxed{\blue{e, \rho, \sigma \Downarrow_{pc}^E \sigma, v}}
    \\
    \begingroup
    \color{\colorMATH}
    \begin{displaymath}
      \begin{array}{c}
        ...
        \begin{array}{ccc}
          \inferrule[\textsc{Box}]
                    {e, \rho, \sigma \Downarrow_{pc}^E v, \sigma' \\\\
                      \alpha \not\in dom(\sigma') \\\\
                      \sigma'' = \sigma' [\alpha \mapsto \formfacet{pc}{v}{\star}]}
                    {(\code{box} ~ e), \rho, \sigma \Downarrow_{pc}^E v, \sigma''}
                    & 
          \inferrule[\textsc{Unbox}]
                    {e, \rho, \sigma \Downarrow_{pc}^E v, \sigma' \\\\
                     v' = \fcol{read(\sigma',v,pc)}}
                    {(\code{unbox} ~e), \rho, \sigma \Downarrow_{pc}^E v', \sigma'}
                    & 
          \inferrule[\textsc{Set}]
                    {e_1, \rho, \sigma \Downarrow_{pc}^E v_1, \sigma' \\\\
                     e_2, \rho, \sigma' \Downarrow_{pc}^E v_2, \sigma'' \\\\
                     \sigma''' = \fcol{write(\sigma'', v_1, pc, v_2)}}
        ...
        \begin{array}{ccc}
          ...
          {e_1, \rho, \sigma \Downarrow_{pc}^E \sigma', v \\\\
           \alpha \not\in dom(\sigma) \and \sigma' = \sigma [\alpha \mapsto v] \\\\
           e_2, \rho [ \ell \mapsto \alpha ], \sigma' \Downarrow_{pc}^E \sigma'', v'}
          {(\code{let-label} ~ \ell ~ e_1 ~ e_2), \rho, \sigma \Downarrow_{pc}^E \sigma'', v'}}
          & 
          {\color{\colorFAC}
          \inferrule[\textsc{Fac-Create}]
          {e_1, \rho, \sigma \Downarrow_{pc}^E \sigma', l \\\\
           e_2, \rho, \sigma' \Downarrow_{pc \cup \{+l\}}^E \sigma'', v_1 \\\\
           e_3, \rho, \sigma'' \Downarrow_{pc \cup \{-l\}}^E \sigma''', v_2 }
          {(\code{fac} ~ e_1 ~ e_2 ~ e_3), \rho, \sigma \Downarrow_{pc}^E \sigma''', \formfacet{pc}{v_1}{v_2}}}
          & 
          {\color{\colorFAC}
          \inferrule[\textsc{Obs}]
          {e_1, \rho, \sigma \Downarrow_{pc}^E \sigma', \ell \\\\
           ...
           e_3, \rho' [x \mapsto v], \sigma'' \Downarrow_{pc}^E \sigma''', v'}
