...
The $\code{(obs}\ \fcol{e_\ell}\ \blue{e_\text{key}}\ \blue{e_\text{fac}}\code{)}$ form takes
a label, a key, and a faceted value. Explicit observation projects a single label
$\fcol{e_\ell}$ in faceted value $\blue{e_\text{fac}}$ to either its positive or negative facet,
depending on whether the policy associated with $\fcol{e_\ell}$ returns true for key
...
\fcol{$\ell_{\text{Friends}}$} and \fcol{$\ell_{\text{Family}}$}. A user
