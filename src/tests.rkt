...
(module+ test
  ...
  (check-equal?
   (let ([x (ref 0)])
     (begin (if (fac con1 #f #t) (set! x (lambda (y) (+ y y))) (set! x (lambda (y) (* y y)))) ((deref x) 3)))
   (fac con1 9 6) "It should return (fac con1 9 6)")

  ; Test case that combines obs, set!, and if.
  (check-equal?
   (let ([x (ref 0)])
     (begin (if (fac con1 #f #t) (set! x 1) (set! x 2)) (obs con1 #t (deref x))))
   2 "It should return 2")

  ; Test cases that combine obs, set!, app and if.
  (check-equal?
   (let ([x (ref 0)])
     (begin (if (fac con1 #f #t) (set! x (lambda (y) (+ y y))) (set! x (lambda (y) (* y y)))) ((obs con1 #t (deref x)) 3)))
   9 "It should return 9")

  (check-equal?
   (let ([x (ref 0)])
     (begin (if (fac con1 #t #f) (set! x (lambda (y) (+ y y))) (set! x (lambda (y) (* y y)))) ((obs con1 #t (deref x)) 3)))
